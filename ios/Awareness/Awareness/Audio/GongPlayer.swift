...
/// iOS version — configures AVAudioSession for playback even in silent mode.
class GongPlayer {
    ...
    private init() {
        // Configure audio session so gong plays even when the device is on silent
        do {
            try AVAudioSession.sharedInstance().setCategory(.playback, mode: .default)
        } catch {
            print("Awareness: failed to configure audio session — \(error.localizedDescription)")
        }
    }
