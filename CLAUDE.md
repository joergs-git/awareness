...
| Blackout | `fullScreenCover` presenting `BlackoutView` with `WKExtendedRuntimeSession` + `ExtendedSessionDelegate` + `WKBackgroundModes: mindfulness` to prevent suspension; hybrid timer: main-thread `Timer.scheduledTimer` (catches up on wrist-raise) + background `DispatchSourceTimer` (fires end haptics directly via `WKInterfaceDevice.play()` bypassing main RunLoop) |
...
- **Display dimming mitigation**: Hybrid timer approach: (1) main-thread `Timer.scheduledTimer` with wall-clock `Date` check catches up on wrist-raise; (2) background `DispatchSourceTimer` fires end haptics directly via `WKInterfaceDevice.current().play()` bypassing the throttled main RunLoop; (3) `TimelineView(.animation(minimumInterval: 1.0))` wrapper signals continuous rendering need to extend display-on time. Note: `WKExtendedRuntimeSession` keeps the process alive but does NOT prevent display dimming — that is Apple's intended behavior. Audio keep-alive (`.ambient` AVAudioEngine) was tried but reverted — the `.ambient` session gets deactivated when the display dims, making things worse.
