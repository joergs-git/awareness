...
| Blackout | `fullScreenCover` presenting `BlackoutView` with `WKExtendedRuntimeSession` + `ExtendedSessionDelegate` + `WKBackgroundModes: mindfulness` to prevent suspension; `DispatchSourceTimer` on background queue for reliable end-of-blackout firing (immune to main RunLoop throttling when display dims) |
...
| Start chime | `ChimePlayer.shared.playStartChimeWithKeepAlive(duration:)` — synthesized 440Hz → 660Hz via `AVAudioEngine` + `AVAudioSourceNode` followed by near-silent keep-alive tone (1Hz, 0.001 volume) for the blackout duration; keeps audio engine active to improve process scheduling priority; `.ambient` audio session respects silent mode; always plays (no toggle) |
...
- `ChimePlayer.shared` uses `AVAudioEngine` + `AVAudioSourceNode` for real-time synthesis; `.ambient` audio session respects watchOS silent mode; `stop()` called in `onDisappear` to clean up. `playStartChimeWithKeepAlive(duration:)` appends a near-silent 1Hz tone (0.001 volume) after the chime to keep the audio engine active for the full blackout — this gives the process audio-app-level scheduling priority, reducing timer throttling when the display dims.
- **Display dimming mitigation**: Three-layer approach: (1) `DispatchSourceTimer` on `DispatchQueue.global(qos: .userInteractive)` for reliable end-of-blackout firing immune to main RunLoop throttling; (2) `TimelineView(.animation(minimumInterval: 1.0))` wrapper signals continuous rendering need to extend display-on time; (3) near-silent audio keep-alive via `ChimePlayer` maintains active audio session for better process scheduling. Note: `WKExtendedRuntimeSession` keeps the process alive but does NOT prevent display dimming — that is Apple's intended behavior.
